  ...
  def checkFormula(formula)
    ...
    @formulaArray.each do |c|
      if c.match(/[0-9]/) != nil then
        ...
        if preC == 's' || preC == 'd' then
          raise StandardError, "Error Message 2 : 演算子、もしくは'.'が不適切に入力されています"
        else
          preC = "s"
        end
      elsif c == 'd' then
        if  preC == 'O' || preC =='s' || preC =='d' then
          raise StandardError, "Error Message 2 : 演算子、もしくは'.'が不適切に入力されています"
        else
          preC = "d"
        end
